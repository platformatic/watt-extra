...
class Watt {
  ...
  constructor (app) {
    ...
    this.areFlamegraphsEnabled = false
  }
  ...
  #configureRuntime (config) {
    ...
    if (this.#env.PLT_DISABLE_FLAMEGRAPHS !== true) {
      try {
        ...
        const pprofPath = this.#require.resolve('@platformatic/wattpm-pprof-capture')
        config.preload.push(pprofPath)
        this.areFlamegraphsEnabled = true
      } catch (err) {
        this.#logger.info(
          '@platformatic/wattpm-pprof-capture module not found, flamegraphs are disabled'
        )
      }
    } else {
      this.#logger.info('PLT_DISABLE_FLAMEGRAPHS is set, flamegraphs are disabled')
    }
