...
class Watt {
        ...
        try {
          ...
          const updatedMetrics = {
            ...
            labels: {
              ...
              ...(instanceConfig.deploymentVersion ? { deploymentVersion: instanceConfig.deploymentVersion } : {})
            },
  ...
  #configureRuntime (config) {
    ...
    const deploymentVersion = this.#instanceConfig?.deploymentVersion
    if (deploymentVersion) {
      labels.deploymentVersion = deploymentVersion
    }
    ...
    // Always load the pprof capture preload: PLT_DISABLE_FLAMEGRAPHS only
    // controls whether profiling starts at boot, and ICC can activate it at
    // runtime on this pod with the start-profiling command. The preload is
    // passive until startProfiling is invoked.
    // The module is resolved from watt-extra's own dependencies. If it is
    // missing anyway (e.g. a stripped install), the pod boots without it:
    // profiling is unavailable and the workers are reported as unsupported.
    let pprofPath = null
    try {
      pprofPath = require.resolve('@platformatic/wattpm-pprof-capture')
    } catch {
      this.#logger.warn(
        '@platformatic/wattpm-pprof-capture is not available, profiling is disabled on this pod'
      )
    }
  ...
  async #configureServices (runtime) {
    ...
    for (const app of config.applications ?? []) {
      // Older runtimes report the bare capability name, current ones the
      // package name (@platformatic/foundation prefixes it): accept both, or
      // the Next cache adapter and the other Next patches are never applied.
      if (app.type === 'next' || app.type === '@platformatic/next') {
